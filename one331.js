(function(){
  ...
  // Фиксация постоянного аппаратного ID Android
  function getHardwareUID(){
    try {
      if (window.AndroidJS && typeof AndroidJS.getDeviceId === 'function') {
        var did = AndroidJS.getDeviceId();
        if (did) return 'ad' + fnv1a(String(did)).substring(0, 6);
      }
    } catch(e) {}
    try {
      if (window.Android && typeof Android.getDeviceId === 'function') {
        var did2 = Android.getDeviceId();
        if (did2) return 'ad' + fnv1a(String(did2)).substring(0, 6);
      }
    } catch(e) {}
    try {
      if (window.webapis && webapis.productinfo && typeof webapis.productinfo.getDuid === 'function') {
        return 'tz' + fnv1a(webapis.productinfo.getDuid()).substring(0, 6);
      }
    } catch(e) {}
    return '';
  }
  ...
  function statusReq(qs,done,fail){
    ...
    var i=0;
    function next(){i++;if(i<hosts.length)attempt();else {if(fail)fail();}}
    function attempt(){
  ...
  // Фоновое автоподтверждение токена после ввода в боте
  var _pollTimer = null;
  function startAuthPolling(){
    ...
    _pollTimer = setInterval(function(){
      ...
      statusReq('uid='+encodeURIComponent(uid)+(cub?'&cub='+encodeURIComponent(cub):''), function(r){
        if (r && r.authorized && r.token) {
          clearInterval(_pollTimer);
          _pollTimer = null;
          saveToken(r.token);
          if (typeof Lampa !== 'undefined' && Lampa.Noty) {
            Lampa.Noty.show('Авторизация Alpac сохранена!');
          }
        }
      });
    }, 3000);
...
(function () {
  ...
  // Хук XHR: подмена адресов Alloha и внедрение токена во все сетевые запросы L-Vid
  ...
  XMLHttpRequest.prototype.open = function(method, url, async, user, pass) {
    if (typeof url === 'string') {
      ...
      // Если запрос идет к L-Vid, гарантированно подмешиваем token и uid
      if (url.indexOf('beta.l-vid.online') !== -1 || url.indexOf('/lite/') !== -1) {
        var tok = (window.__alcopacSaveToken && typeof getToken === 'function') ? getToken() : '';
        var uid = window.__alcopacEnsureUID ? window.__alcopacEnsureUID() : '';
        if (tok && url.indexOf('token=') === -1) {
      ...
      Lampa.Reguest.prototype.silent = function(url, success, error, post, options) {
        if (typeof url === 'string') {
          ...
          if (url.indexOf('beta.l-vid.online') !== -1) {
            ...
            var uid = window.__alcopacEnsureUID ? window.__alcopacEnsureUID() : '';
            if (tok && url.indexOf('token=') === -1) url += (url.indexOf('?') >= 0 ? '&' : '?') + 'token=' + encodeURIComponent(tok);
            if (uid && url.indexOf('uid=') === -1) url += (url.indexOf('?') >= 0 ? '&' : '?') + 'uid=' + encodeURIComponent(uid);
          }
        }
  ...
  function loadBundle() {
    var timer = setInterval(function () {
      if (typeof Lampa !== 'undefined') {
        ...
        // Фиксация постоянного аппаратного UID
        ...
        for (var i = 0; i < LIST.length; i++) {
          ...
          var scriptUrl = p.u;
          // Дописываем токен и uid в URL скриптов, чтобы они стартовали авторизованными
          if (mtok && scriptUrl.indexOf('beta.l-vid.online') !== -1) {
            scriptUrl += (scriptUrl.indexOf('?') >= 0 ? '&' : '?') + 'token=' + encodeURIComponent(mtok) + '&uid=' + encodeURIComponent(unic_id);
          }
          urls.push(scriptUrl);
        }
