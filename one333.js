...
// --- Глобальные хуки балансеров Alloha и Torrent (ab2024.ru) ---
(function(){
  'use strict';
  ...
  // Перехват XHR: подмена путей к ab2024.ru
  ...
  XMLHttpRequest.prototype.open = function(method, url, async, user, pass) {
    if (typeof url === 'string') {
      if (url.indexOf('alloha') !== -1 || url.indexOf('pidtor') !== -1 || url.indexOf('lite/torrent') !== -1) {
        url = url.replace(/https?:\/\/[^\/]+/i, ALLOHA_HOST);
  ...
  // Хуки интерфейса Lampa (эмодзи и фильтры)
  var hookInterval = setInterval(function() {
    ...
    if (Lampa.Reguest) {
      ...
      Lampa.Reguest.prototype.silent = function(url, success, error, post, options) {
        if (typeof url === 'string' && (url.indexOf('alloha') !== -1 || url.indexOf('pidtor') !== -1 || url.indexOf('lite/torrent') !== -1)) {
          url = url.replace(/https?:\/\/[^\/]+/i, ALLOHA_HOST);
          url = url.replace('/lite/torrent', '/lite/pidtor');
        }
