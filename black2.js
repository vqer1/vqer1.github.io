// ─── LAMPAC HUD DEBUGGER & SCRIPT INTERCEPTOR ───────────────────────
(function() {
    if (window.__lampac_debug_injected) return;
    window.__lampac_debug_injected = true;

    // Создаем окно логов в DOM
    var hud = $(
        '<div id="lampa-hud-log" style="' +
        'position:fixed; bottom:10px; right:10px; width:460px; height:240px; ' +
        'background:rgba(10,10,15,0.92); border:1px solid #ff9800; border-radius:6px; ' +
        'color:#eee; font-family:monospace; font-size:11px; z-index:999999; ' +
        'display:flex; flex-direction:column; box-shadow:0 0 10px rgba(0,0,0,0.8);">' +
        '  <div style="background:#222; padding:4px 8px; font-weight:bold; color:#ff9800; display:flex; justify-content:space-between;">' +
        '    <span>Lampac Network & Script Monitor</span>' +
        '    <span style="cursor:pointer;" onclick="$(\'#lampa-hud-content\').empty()">[Clear]</span>' +
        '  </div>' +
        '  <div id="lampa-hud-content" style="flex:1; overflow-y:auto; padding:6px; white-space:pre-wrap; word-break:break-all;"></div>' +
        '</div>'
    );
    $('body').append(hud);

    function printLog(tag, msg, color) {
        var box = $('#lampa-hud-content');
        var row = $('<div style="margin-bottom:3px;"></div>');
        row.append('<span style="color:' + (color || '#00e676') + '; font-weight:bold;">[' + tag + '] </span>');
        row.append('<span>' + (typeof msg === 'object' ? JSON.stringify(msg) : msg) + '</span>');
        box.append(row);
        box.scrollTop(box[0].scrollHeight);
    }

    // 1. Перехват динамически загружаемых скриптов
    var origPutScript = Lampa.Utils.putScript;
    Lampa.Utils.putScript = function(scripts, onload, onerror, oncall, nomanifest) {
        printLog('LOAD_SCRIPT', scripts, '#29b6f6');
        return origPutScript.apply(this, arguments);
    };

    var origPutScriptAsync = Lampa.Utils.putScriptAsync;
    Lampa.Utils.putScriptAsync = function(scripts, oncomplite) {
        printLog('LOAD_SCRIPT_ASYNC', scripts, '#ab47bc');
        return origPutScriptAsync.apply(this, arguments);
    };

    // 2. Перехват Native AJAX для анализа 4K ссылок и accsdb
    var origNative = Lampa.Reguest.prototype.native;
    Lampa.Reguest.prototype.native = function(url, onsuccess, onerror, post, params) {
        printLog('REQ', url.split('?')[0] + ' (memkey/auth scan)', '#ffd54f');

        var wrappedSuccess = function(data) {
            try {
                var json = typeof data === 'string' ? JSON.parse(data) : data;
                
                // Проверка на серверную блокировку доступа
                if (json.accsdb) {
                    printLog('BLOCKED', 'accsdb: ' + (json.denymsg || json.msg || 'Denied'), '#ff5252');
                }

                // Сканирование доступных качеств
                if (json.qualitys || json.quality) {
                    var q = Object.keys(json.qualitys || json.quality);
                    printLog('QUALITIES', q.join(', '), q.indexOf('2160') !== -1 ? '#00e676' : '#ffa726');
                }
            } catch(e) {}
        ...
        return origNative.call(this, url, wrappedSuccess, onerror, post, params);
    };
})();
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(
        account('https://akter-black.com/lite/withsearch'),
        function (json) {
